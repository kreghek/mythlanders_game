...
internal sealed class NotificationUiElement : ControlBase
{
    ...
    public NotificationUiElement(Texture2D texture, TextureRegion2D icon, string notificationType,
        string notificationText) : base(texture)
    {
        _stackPanelElement = new HorizontalStackPanel(texture, ControlTextures.Transparent, new ControlBase[]
        {
            new Image(icon.Texture, icon.Bounds, UiThemeManager.UiContentStorage.GetControlBackgroundTexture(),
        ...
        _background = new BackgroundImage(texture, ControlTextures.PanelBlack,
    ...
    {
        return Color.Lerp(Color.White, Color.Transparent, 1 - Lifetime);
    }

    protected override void DrawContent(SpriteBatch spriteBatch, Rectangle contentRect, Color contentColor)
    {
        _background.Rect = new Rectangle(contentRect.Left + 4, contentRect.Top + 4, contentRect.Width - 4 * 2,
            contentRect.Height - 4 * 2);
