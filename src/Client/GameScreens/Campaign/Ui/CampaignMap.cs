...
internal sealed class CampaignMap : ControlBase
{
        ...
        GameObjectContentStorage gameObjectContentStorage)
    {
        ...
        InitChildControls(heroCampaign.Source.Stages, heroCampaign);
    }
        ...
        IGraphNodeLayout<ICampaignStageItem> graphNodeLayout)
    {
        ...
        button.OnClick += (_, _) =>
        {
            ...
            if (_heroCampaign.CurrentStage is null)
            {
                ...
                var roots = GetRoots(_heroCampaign.Source.Stages);
            ...
            {
                if (_heroCampaign.Source.Stages.GetNext(_heroCampaign.CurrentStage).Contains(graphNodeLayout.Node))
                {
                    SelectCampaignStage(graphNodeLayout.Node, transitionData);
                }
            }
        ...
        IReadOnlyCollection<IGraphNodeLayout<ICampaignStageItem>> graphNodeLayouts)
    {
        ...
        if (currentCampaign.Path.Any())
        {
            ...
            var next = currentCampaign.Source.Stages.GetNext(prevNodeLayout.Node);
    ...
    private void DrawEdges(SpriteBatch spriteBatch)
    {
        foreach (var button in _buttonList)
        {
            var nextNodes = _heroCampaign.Source.Stages.GetNext(button.SourceGraphNodeLayout.Node);
    ...
    private void DrawNodeHighlights(SpriteBatch spriteBatch)
    {
        ...
        else
        {
            ...
            var next = _heroCampaign.Source.Stages.GetNext(_heroCampaign.CurrentStage);
    ...
    private CampaignNodeState GetLocationNodeState(IGraphNodeLayout<ICampaignStageItem> graphNodeLayout)
    {
        ...
        {
            var availableNodes = _heroCampaign.Source.Stages.GetAvailableNodes(_heroCampaign.CurrentStage);
