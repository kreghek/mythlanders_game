...
public sealed class LifeDrawEffectInstance : EffectInstanceBase<LifeDrawEffect>
{
    ...
    public override void Influence(ICombatant target, ICombatMovementContext context)
    {
        ...
        var damageRemains = context.DamageCombatantStat(target, new CombatantDamageSource(context.Actor), CombatantStatTypes.ShieldPoints, absorbedDamage);

        if (damageRemains > 0)
        {
            //TakeStat(target, UnitStatType.HitPoints, damageRemains);
            var stolenHitPoints = context.DamageCombatantStat(target, new CombatantDamageSource(context.Actor), CombatantStatTypes.HitPoints, damageRemains);
