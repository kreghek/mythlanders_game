...
public sealed class StrongestClosestAllyTargetSelector : ITargetSelector
{
    public IReadOnlyList<Combatant> GetMaterialized(Combatant actor, ITargetSelectorContext context)
    {
        ...
        var sortedCombatants = neighbourCombatants.OrderByDescending(x => x.proprity).ThenByDescending(x => x.combatant.Stats.Single(s => s.Type == UnitStatType.HitPoints).Value.Current).Select(x => x.combatant).ToArray();
