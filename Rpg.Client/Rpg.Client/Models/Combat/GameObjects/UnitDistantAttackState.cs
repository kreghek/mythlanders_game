...
namespace Rpg.Client.Models.Combat.GameObjects
{
    internal class UnitDistantAttackState : IUnitStateEngine
    {
        ...
        public UnitDistantAttackState(UnitGraphics graphics, SpriteContainer graphicsRoot,
            ...
            BulletGameObject? bullet, IList<IInteractionDelivery> interactionDeliveryList, SoundEffectInstance hitSound, int index)
        {
            var targetPosition =
                targetGraphicsRoot.Position + new Vector2(-100 * (targetGraphicsRoot.FlipX ? 1 : -1), 0);
            _subStates = new IUnitStateEngine[]
            {
                new DistantHitState(graphics, attackInteraction, bullet, interactionDeliveryList, blocker, hitSound, index)
            };
            _blocker = blocker;
        }
