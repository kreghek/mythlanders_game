...
namespace Rpg.Client.GameScreens.Locations.Ui
{
    internal sealed class CombatModal : ModalDialogBase
    {
        ...
        private readonly GlobeNode _nodeGameObject;
            ...
            resolutionIndependentRenderer)
        {
            ...
            _nodeGameObject = context.GlobeNode;
            ...
            {
                context.CombatDelegate(_nodeGameObject);
            };
        ...
        {
            if (_nodeGameObject.CombatSequence is null)
            {
        ...
        private void AddAutoCombatButtonIfAvailable(CombatModalContext context)
        {
            ...
            if (isAutoCombatAvailable)
            {
                ...
                {
                    context.AutoCombatDelegate(context.GlobeNode);
                };
        ...
        private void DisplayCombatRewards(SpriteBatch spriteBatch, GlobeNode nodeGameObject,
            Vector2 rewardBlockPosition, GlobeNode node)
        {
        ...
        private void DrawCombatShortInfo(SpriteBatch spriteBatch, Rectangle contentRectangle)
        {
            ...
            var localizedName = GameObjectHelper.GetLocalized(_nodeGameObject.Sid);

            spriteBatch.DrawString(_uiContentStorage.GetTitlesFont(), localizedName,
                textPosition + new Vector2(5, 5),
                Color.Wheat);

            var dialogMarkerText = _nodeGameObject.AssignedEvent is not null
                ? $"{_nodeGameObject.AssignedEvent.Title}"
                : string.Empty;
            spriteBatch.DrawString(_uiContentStorage.GetMainFont(), dialogMarkerText,
                textPosition + new Vector2(5, 25), Color.Wheat);

            var combatCount = _nodeGameObject.CombatSequence.Combats.Count;
            var combatSequenceSizeText = LocationsScreenTextHelper.GetCombatSequenceSizeText(combatCount);
            ...
            var uniqueMonsterSchemes = _nodeGameObject.CombatSequence.Combats
        ...
        private void DrawEquipmentRewards(SpriteBatch spriteBatch,
            GlobeNode nodeGameObject, Vector2 toolTipPosition)
        {
            var equipmentType = nodeGameObject.EquipmentItem;
        ...
        private void DrawSummaryXpAwardLabel(SpriteBatch spriteBatch, GlobeNode node, Vector2 toolTipPosition)
        {
            var totalXpForMonsters = node.CombatSequence.Combats.SelectMany(x => x.EnemyGroup.GetUnits())
                .Sum(x => x.XpReward);
