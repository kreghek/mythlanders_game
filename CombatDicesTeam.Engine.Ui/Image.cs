...
public sealed class Image : ControlBase
{
    ...
    public Image(Texture2D image, Rectangle sourceRect, Texture2D texture, Point textureOffset) : this(image,
        sourceRect, texture, textureOffset, () => Color.White, null)
    {
    }

    public Image(Texture2D image, Rectangle sourceRect, Texture2D texture, Point textureOffset,
        Func<Color> colorDelegate, Point? size) : base(texture)
    {
        _image = image;
